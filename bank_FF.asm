...
.export sub_0x01D072_обработка_яркости_с_ожиданием_завершения_обработки
.export sub_0x01D083_обработка_текущей_яркости
...
C - - - - - 0x01C504 07:C4F4: 20 62 D0  JSR sub_D062_обработка_яркости_с_ожиданием_завершения_обработки
...
C - - - - - 0x01C557 07:C547: A9 02     LDA #$01
...
bra_C706:
loc_C706:
C D 2 - - - 0x01C716 07:C706: 20 73 D0  JSR sub_D073_обработка_текущей_яркости
...
C D 2 - - - 0x01D064 07:D054: A9 02     LDA #$01
...
sub_0x01D072_обработка_яркости_с_ожиданием_завершения_обработки:
sub_D062_обработка_яркости_с_ожиданием_завершения_обработки:
bra_D062_infinite_loop:
C D 2 - - - 0x01D072 07:D062: 20 73 D0  JSR sub_D073_обработка_текущей_яркости
...
sub_0x01D083_обработка_текущей_яркости:
sub_D073_обработка_текущей_яркости:
; свободные адреса 001C 002C 002D 0030 0031
C D 2 - - - 0x01D083 07:D073: 2C B2 05  BIT ram_флаг_яркости
C - - - - - 0x01D086 07:D076: 30 52     BMI bra_D0CA_RTS    ; если яркость готова
                                        LDA ram_счетчик_яркости
                                        BVC bra_D08E_идет_отсветление
                                        EOR #$FF
                                        CLC
                                        ADC #$11
bra_D08E_идет_отсветление:
...
@bra_цикл_копирования_в_ppu:
                                        ...
                                        LDA ram_002E
                                        CMP #$FF
                                        BNE @bra_скопировать_в_ppu
                                        DEC ram_002F
                                        LDA ram_002F
                                        CMP #$FF
                                        BNE @bra_скопировать_в_ppu
                                        RTS
@bra_скопировать_в_ppu:
                                        LDA (ram_002C),Y
                                        STA $2007
                                        INC ram_002C
                                        BNE @bra_цикл_копирования_в_ppu
                                        INC ram_002D
                                        BMI @bra_цикл_копирования_в_ppu     ; старший байт всегда 80+
...
; 00 = vertical, 01 = horisontal
    ...
    .byte $00, con_prg_bank + $08   ; 10
    .byte $00, con_prg_bank + $08   ; 11
    .byte $00, con_prg_bank + $08   ; 12
    ...
    .word $FFFF                     ; 2E
    .word $FFFF                     ; 2F
    .byte $01, con_prg_bank + $0B   ; 30
    .byte $01, con_prg_bank + $0B   ; 31
    .byte $01, con_prg_bank + $0B   ; 32
    .byte $01, con_prg_bank + $0B   ; 33
    .byte $01, con_prg_bank + $0B   ; 34
    .byte $01, con_prg_bank + $0B   ; 35
    .byte $01, con_prg_bank + $0B   ; 36
    .byte $01, con_prg_bank + $0B   ; 37
    .byte $01, con_prg_bank + $0B   ; 38
    .byte $01, con_prg_bank + $0B   ; 39
    .byte $01, con_prg_bank + $0B   ; 3A
    .byte $01, con_prg_bank + $0B   ; 3B
    .byte $01, con_prg_bank + $0B   ; 3C
    .byte $01, con_prg_bank + $0B   ; 3D
    .byte $01, con_prg_bank + $0B   ; 3E
    .byte $01, con_prg_bank + $0B   ; 3F
...
tbl_0001_начальный_адрес_байтов_экрана:
    ...
    .word $FFFF                 ; 35
    .word $FFFF                 ; 36
    .word _off000_screen_37     ; 37
    .word $FFFF                 ; 38
    .word _off000_screen_39     ; 39
    .word $FFFF                 ; 3A
    .word _off000_screen_3B     ; 3B
    .word $FFFF                 ; 3C
    .word _off000_screen_3D     ; 3D
    .word _off000_screen_3E     ; 3E
    .word $FFFF                 ; 3F
...
C - - - - - 0x01F072 07:F062: 30 4F     BMI bra_F0B3_RTS
                                        CMP #$3F
                                        BEQ bra_F0AE_пропустить_запись_оригинальной_палитры
...
bra_F0AE_пропустить_запись_оригинальной_палитры:
...
@loc_запись_номера_цвета:
                                        STY $401B
                                        LDA ram_color_R,Y
                                        JSR sub_0002_скорректировать_яркость
                                        STA $401B
                                        LDA ram_color_G,Y
                                        JSR sub_0002_скорректировать_яркость
                                        STA $401B
                                        ...
                                        LDA ram_000C
                                        CMP ram_pal_buffer
                                        BNE @bra_цикл_одного_цвета
                                        LDA #$00    ; закрыть буфер
                                        STA ram_pal_buffer
@bra_выход:
                                        RTS

sub_0002_скорректировать_яркость:
                                        STA ram_000D
                                        LDA ram_степень_яркости
                                        BEQ @bra_нулевая_яркость
                                        CMP #$10
...
@bra_максимальная_яркость:  ; не изменять яркость
                                        LDA ram_000D
@bra_нулевая_яркость:       ; A = 00
                                        RTS



sub_0003_обновить_палитру_фона:
                                        LDA $BFFF
                                        PHA
                                        LDA #con_prg_bank + $0C
                                        JSR sub_EED3_prg_bankswitch
                                        JSR sub_0x000001_обновить_палитру_фона
                                        JMP loc_C368_восстановить_prg_банк
...
C - - - - - 0x01FC47 07:FC37: 20 08 C3  JSR sub_C308_обновить_звуковой_движок
                                        JSR sub_0003_обновить_палитру_фона
